...
def partTwo(fieldsDict, othersTicketsList ):
    """
    Find field order.
    :param fieldsDict:
    :param othersTicketsList:
    ...
    for fieldName, fieldIntervals in fieldsDict.items():
        ...
        potentialIndexs =  [i for i in range(len(fieldsDict.keys()))]
...
if __name__ == "__main__":
    ...
    fieldMaping = partTwo(fieldsDict, allTickets )
    ...
    for key, value in fieldMaping.items():
        ...
        if "departure" in key:
            index = value.pop()
            resultNumber *= ourTicketList[index]

    print(f"Part 2: resultNumber: {resultNumber}")
